...
| Intel Gaudi HPU 1.17          | `BUILD_CUDA_EXT=0 pip install auto-gptq --no-build-isolation` | [2.3.0+Intel Gaudi 1.17](https://docs.habana.ai/en/latest/Installation_Guide/)               |
...
#### On Intel Gaudi systems

To install from source for Intel Gaudi HPUs, set environment variable `BUILD_CUDA_EXT=0` to disable CUDA pytorch extension building. Example:

```bash
BUILD_CUDA_EXT=0 pip install -vvv --no-build-isolation -e .
```

>Notice that Intel Gaudi uses an optimized kernel upon inference, and requires `BUILD_CUDA_EXT=0` on non-CUDA machines.
