...
if BUILD_CUDA_EXT:
    try:
        ...
        print(
            f"Building cuda extension requires PyTorch (>=1.13.0) being installed, please install PyTorch first: {e}"
        )
        sys.exit(1)
    ...
    if platform.system() != "Windows":
        ...
        if not ROCM_VERSION and COMPILE_MARLIN:
            ...
            if not torch_cuda_archs:
                raise ValueError(
                    'The environment variable `TORCH_CUDA_ARCH_LIST` needs to be specified to compile AutoGPTQ with Marlin kernel. Example: `TORCH_CUDA_ARCH_LIST="8.0 8.6+PTX"`.'
                )

            archs_list = torch_cuda_archs.split(" ")
            if any(arch.startswith("6") or arch.startswith("7") for arch in archs_list):
                raise ValueError(
                    'Marlin kernel can not be compiled CUDA compute capability <8.0. Please specifiy a correct `TORCH_CUDA_ARCH_LIST` environment variable. Example: `TORCH_CUDA_ARCH_LIST="8.0 8.6+PTX"`'
                )
