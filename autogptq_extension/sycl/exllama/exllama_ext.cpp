...
#include "sycl_func/q4_matrix.hpp"
#include "sycl_func/q4_matmul.hpp"
#include "sycl_func/column_remap.hpp"
...
do { \
    ...
    TORCH_CHECK(__index < SYCL_MAX_DEVICES, "invalid device index"); \
} while(0)
...
)
{
    ...
    if (tuningParams.matmul_recons_thd == 0 || x_height < tuningParams.matmul_recons_thd)
    {
        ...
        (
            &tuningParams,
            (sycl::half*) x.data_ptr(),
            x_height,
            wm,
            (sycl::half*) out.data_ptr()
        );
    }
    else
    {
        q4_matmul_recons_sycl
        (
            &tuningParams,
            (sycl::half*) x.data_ptr(),
            x_height,
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, m)
{
    ...
    m.def("cleanup_buffers_sycl", &cleanup_buffers_sycl, "cleanup_buffers_sycl");
}
