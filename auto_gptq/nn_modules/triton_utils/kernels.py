...
def quant_matmul_248_inference_only(input, qweight, scales, qzeros, g_idx, bits, maxq):
    with torch.cuda.device(input.device):
...
class QuantLinearInferenceOnlyFunction(torch.autograd.Function):
    ...
    def forward(ctx, input, qweight, scales, qzeros, g_idx, bits, maxq):
        output = quant_matmul_248_inference_only(input, qweight, scales, qzeros, g_idx, bits, maxq)
        return output
